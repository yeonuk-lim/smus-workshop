...
아래 질문들도 시도해보세요. 단순 센서 조회가 아니라 에이전트가 **상황을 종합 판단하고 스스로 행동을 결정**하는 과정을 볼 수 있습니다.

```
지금 강아지 혼자 있어? 괜찮을 것 같아?
```
> 주인 재실 여부 + 강아지 불안도를 종합해서 "괜찮다 / 조치가 필요하다"를 에이전트가 직접 판단합니다.

```
내가 2시간째 외출 중인데, 강아지 상태 어때? 뭔가 해줘야 해?
```
> 컨텍스트(외출 중)를 주었을 때 에이전트가 센서를 확인하고 필요하면 음악 재생까지 결정하는 흐름을 볼 수 있습니다.

```
강아지가 요즘 예민한 것 같아. 지금 상태 보고 내가 뭘 해줄 수 있는지 알려줘
```
> 불안도 수준에 따라 에이전트가 이미 조치했으면 "재생 중"을, 낮으면 "현재 안정적"을 설명하는 등 상황별로 다른 답변을 생성합니다.
...
물 부족이면 level='low', 충분하면 'ok'를 반환하는 Mock으로 만들어줘.
```
...
> 시스템 프롬프트는 **목표와 제약**만 담는 게 좋습니다. "1번 하고 2번 하고…" 식으로 순서를 명시하면 `if-else` 자동화와 다를 게 없어집니다. 이 에이전트의 `SYSTEM_PROMPT`처럼 "상황을 스스로 파악해라" 방식이 진짜 에이전트다운 접근입니다.
