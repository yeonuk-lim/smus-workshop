...
> **목표**: 미션 1까지 완성된 에이전트를 **AgentCore Runtime에 배포**하고, CopilotKit 웹UI가 **로컬 에이전트가 아닌 클라우드 에이전트**에 연결되도록 만드세요.
...
cd ~/smus-petcare-agent/agent
source .venv/bin/activate
pip install bedrock-agentcore-starter-toolkit   # 아직 설치 안 했으면
./deploy-agentcore.sh
```

> `✅ 완료!` 메시지가 나올 때까지 기다립니다 (수 분 소요).
> 스크립트가 끝나면 `frontend/.env.local`이 **클라우드 연결 정보로 자동 교체**됩니다.

#### Step 2 — 웹UI를 클라우드에 연결

```bash
cd ~/smus-petcare-agent/frontend
npm run dev   # 이미 실행 중이면 Ctrl+C 후 재시작
```

브라우저에서 **http://localhost:3300** 접속 → 채팅창에 입력:

```
강아지 상태 점검해줘
```

#### Step 3 — 클라우드 에이전트인지 확인

로컬 에이전트(`python main.py`)를 **종료**해도 채팅이 동작하면 성공입니다.

### 완료 기준

- [ ] `deploy-agentcore.sh` 완료 → `frontend/.env.local`에 `AGENTCORE_RUNTIME_ARN` 기록됨
- [ ] 로컬 에이전트 없이 브라우저 채팅에서 에이전트 응답 수신
- [ ] (보너스) 미션 1에서 추가한 Tool도 클라우드에서 동작하는지 확인

> 🔁 **로컬로 되돌리기**: `frontend/.env.local`에서 `AGENTCORE_RUNTIME_ARN`으로 시작하는 줄들을 모두 지우고 `AGENT_URL=http://localhost:8080` 한 줄만 남기면 됩니다.
...
- [ ] **[미션 2]** 웹UI가 클라우드 에이전트에 연결되어 채팅 동작 확인 (로컬 에이전트 없이)
